...
// Функція виводить сповіщення на екран (у реальному часі)
function showMessage(message) {
   let output = document.getElementById("game-output")
   output.innerHTML += `<br>${message}`
   window.scrollTo(0, document.body.scrollHeight) // Автопрокрутка вниз
}
...
function playGame(battlefield) {
   ...
   function nextShot() {
       if (remainingShips === 0) {
           showMessage(`<br>Гру завершено! Всі кораблі потоплено за ${shots} пострілів.`)
           return
       }

       setTimeout(() => { // Використовуємо `setTimeout()`, щоб дати браузеру оновити сторінку
           let shot = getUserShot(battlefield.length)
           if (shot === null) return // Вихід із гри
           ...
           nextShot() // Рекурсивно викликаємо наступний постріл
       }, 100) // Додаємо невелику затримку, щоб браузер встиг оновити сторінку
   }
